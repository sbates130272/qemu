...
/* DMA command bridge is now provided by generic pci_command_bridge.h */
...
/* DMATargetRegion is now provided by pci_command_bridge.h as PCICommandRegion */
...
struct PCITestDevState {
    ...
    /* Generic PCI command bridge */
    PCICommandBridgeState cmd_bridge;
 ...
 */
/*
 * BAR resolver callback for command bridge
 */
static MemoryRegion *pci_testdev_get_bar(PCIDevice *pci_dev, uint8_t bar_num)
{
    ...
    case 1:
        return &d->portio;
    case 2:
        return (d->membar_size > 0) ? &d->membar : NULL;
    default:
        return NULL;
    }
}
...
static void pci_testdev_realize(PCIDevice *pci_dev, Error **errp)
{
    ...
    if (pci_command_bridge_init(pci_dev, &d->cmd_bridge,
                                 pci_testdev_get_bar, errp) < 0) {
        return;
    }
...
pci_testdev_uninit(PCIDevice *dev)
{
    ...
    pci_command_bridge_cleanup(&d->cmd_bridge);
...
static void qdev_pci_testdev_reset(DeviceState *dev)
{
    ...
    /* Reset command bridge */
    pci_command_bridge_reset(&d->cmd_bridge);
...
static const Property pci_testdev_properties[] = {
    ...
    DEFINE_PCI_COMMAND_BRIDGE_PROPERTIES(PCITestDevState, cmd_bridge),

    /* Doorbell DMA engine properties */
