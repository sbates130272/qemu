 ...
 *       -I/usr/include/libdrm -ldrm -ldrm_amdgpu -lhsa-runtime64
 *   Or simply: make
 *
 * Run (as root):
...
__global__ void gpu_fill_lfsr_pattern(uint32_t *buffer, uint32_t size_words, uint32_t seed)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= size_words) return;
    
    /* 32-bit LFSR (Galois form): x^32 + x^22 + x^2 + x + 1 */
    uint32_t lfsr = seed;  /* Use provided seed */
...
__global__ void gpu_submit_nvme_write(
    void *bridge_or_doorbell,   /* MMIO bridge buffer OR direct doorbell address */
    ...
    bool use_direct_doorbell,   /* True = write direct, False = use bridge */
    uint16_t nvme_bdf,          /* NVMe device BDF (for bridge mode) */
    uint8_t nvme_bar,           /* NVMe doorbell BAR (for bridge mode) */
    uint32_t doorbell_offset)   /* Doorbell register offset */
{
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        ...
        if (use_direct_doorbell) {
            /* Direct mode: Write directly to NVMe doorbell register */
            volatile uint32_t *doorbell_reg = 
                (volatile uint32_t *)((char *)bridge_or_doorbell + doorbell_offset);
            
            __threadfence_system();  /* Ensure all prior writes are visible */
            *doorbell_reg = new_sq_tail;
            __threadfence_system();  /* Ensure doorbell write completes */
            
        } else {
            /* Bridge mode: Request MMIO bridge to write doorbell */
            struct pci_mmio_ring_meta *meta = (struct pci_mmio_ring_meta *)bridge_or_doorbell;
            ...
            struct pci_mmio_command *bridge_cmd = 
                (struct pci_mmio_command *)((char *)bridge_or_doorbell + 16 + slot * sizeof(struct pci_mmio_command));
            
            /* Write to NVMe doorbell register via bridge */
 ...
 */
static void *map_nvme_bar0(uint16_t nvme_bdf, int drm_fd)
{
    char path[256];
    int bar0_fd;
    void *bar0_cpu = NULL;
    ...
    /* Open NVMe BAR0 resource file */
    snprintf(path, sizeof(path),
             "/sys/bus/pci/devices/0000:%02x:%02x.%x/resource0",
             bus, dev, func);
    
    bar0_fd = open(path, O_RDWR | O_SYNC);
    if (bar0_fd < 0) {
        fprintf(stderr, "ERROR: Cannot open NVMe BAR0: %s\n", strerror(errno));
        fprintf(stderr, "  Path: %s\n", path);
        fprintf(stderr, "  Make sure the device is accessible and you have permissions\n");
        return NULL;
    }
    
    /* Get BAR0 size */
    off_t bar0_size = lseek(bar0_fd, 0, SEEK_END);
    if (bar0_size < 0) {
        fprintf(stderr, "ERROR: Cannot get BAR0 size\n");
        close(bar0_fd);
        return NULL;
    }
    lseek(bar0_fd, 0, SEEK_SET);
    
    printf("  NVMe BAR0 size: %ld bytes\n", bar0_size);
    
    /* Memory-map BAR0 */
    bar0_cpu = mmap(NULL, bar0_size, PROT_READ | PROT_WRITE, MAP_SHARED, bar0_fd, 0);
    close(bar0_fd);  /* Can close fd after mmap */
    
    if (bar0_cpu == MAP_FAILED) {
        fprintf(stderr, "ERROR: Failed to mmap NVMe BAR0: %s\n", strerror(errno));
        return NULL;
    }
    
    printf("  BAR0 mapped at CPU address: %p\n", bar0_cpu);
    
    /* Register BAR0 with GPU via GEM_USERPTR */
    userptr.addr = (uint64_t)bar0_cpu;
    userptr.size = bar0_size;
    userptr.flags = AMDGPU_GEM_USERPTR_REGISTER;
    
    if (ioctl(drm_fd, DRM_IOCTL_AMDGPU_GEM_USERPTR, &userptr) < 0) {
        fprintf(stderr, "ERROR: GEM_USERPTR failed for BAR0: %s\n", strerror(errno));
        fprintf(stderr, "  GPU may not be able to access NVMe BAR directly\n");
        munmap(bar0_cpu, bar0_size);
        return NULL;
    }
    
    gem_handle = userptr.handle;
    printf("  ✅ BAR0 registered with GPU (GEM handle: %u)\n", gem_handle);
...
/* Export VRAM allocation as DMA-BUF using HSA runtime 
 * Based on: https://github.com/linux-rdma/perftest/blob/master/src/rocm_memory.c
 */
static int export_vram_as_dmabuf(void *vram_ptr, size_t size, int *dmabuf_fd, uint64_t *dmabuf_offset)
{
    hsa_status_t status;
    ...
    offset = (uintptr_t)vram_ptr - (uintptr_t)aligned_ptr;
    aligned_size = (size + offset + host_page_size - 1) & ~(host_page_size - 1);
    
    printf("  Aligning buffer: ptr=%p -> aligned=%p, size=%zu -> %zu\n",
           vram_ptr, (void*)aligned_ptr, size, aligned_size);
    
    /* Export as DMA-BUF using HSA runtime */
    *dmabuf_fd = 0;
    status = hsa_amd_portable_export_dmabuf((void*)aligned_ptr, aligned_size, dmabuf_fd, &offset);
    if (status != HSA_STATUS_SUCCESS) {
        fprintf(stderr, "ERROR: hsa_amd_portable_export_dmabuf failed (status=%d)\n", status);
    ...
    *dmabuf_offset = offset;
    printf("  ✅ Exported dmabuf: fd=%d, offset=0x%lx\n", *dmabuf_fd, offset);
    return 0;
}
...
/* IOCTL structures for nvme_vram_dmabuf kernel module */
...
/* Get physical address from DMA-BUF using kernel module
 * Uses kernel P2PDMA infrastructure via dma_buf_attach/map_attachment
 */
static int get_dmabuf_physical_address(int dmabuf_fd, uint16_t nvme_bdf,
                                       uint64_t dmabuf_offset, 
                                       size_t size, uint64_t *phys_addr)
{
    int kmod_fd;
    struct nvme_vram_dmabuf_req req = {0};
    
    /* Open kernel module device */
    kmod_fd = open("/dev/nvme_vram_dmabuf", O_RDWR);
    if (kmod_fd < 0) {
        fprintf(stderr, "ERROR: Cannot open /dev/nvme_vram_dmabuf: %s\n", strerror(errno));
        fprintf(stderr, "  Load the kernel module first:\n");
        fprintf(stderr, "    cd pci-mmio-guest-code/kernel && make\n");
        fprintf(stderr, "    sudo insmod nvme_vram_dmabuf.ko emulated_nvme_bdf=0x%04x\n", nvme_bdf);
        fprintf(stderr, "  (Use emulated_nvme_bdf=0x%04x for emulated NVMe, omit for passthrough)\n", nvme_bdf);
        return -1;
    }
    
    /* Request physical address via ioctl 
     * Kernel module automatically detects emulated vs passthrough based on:
     *   1. emulated_nvme_bdf module parameter (if set)
     *   2. flags field in request (optional override)
     */
    req.dmabuf_fd = dmabuf_fd;
    req.nvme_bdf = nvme_bdf;
    req.flags = 0;  /* Let kernel module param decide */
    
    if (ioctl(kmod_fd, NVME_VRAM_GET_PHYS_ADDR, &req) < 0) {
        fprintf(stderr, "ERROR: NVME_VRAM_GET_PHYS_ADDR ioctl failed: %s\n", strerror(errno));
        fprintf(stderr, "  If using emulated NVMe, load module with:\n");
        fprintf(stderr, "    sudo insmod nvme_vram_dmabuf.ko emulated_nvme_bdf=0x%04x\n", nvme_bdf);
        close(kmod_fd);
        return -1;
    }
    ...
    *phys_addr = req.phys_addr + dmabuf_offset;
    printf("  ✅ Got P2PDMA address via kernel module: 0x%016lx\n", *phys_addr);
...
int main(int argc, char **argv)
{
    ...
    void *bridge_gpu = NULL, *sq_gpu = NULL, *nvme_bar0_cpu = NULL;
    ...
    bool use_direct_doorbell = false;
    uint32_t lfsr_seed = 0xACE1;
    
    /* Simple argument parsing */
    int arg_idx = 1;
    while (arg_idx < argc) {
        if (strcmp(argv[arg_idx], "--device") == 0 || strcmp(argv[arg_idx], "-d") == 0) {
            use_device_memory = true;
            arg_idx++;
        } else if (strcmp(argv[arg_idx], "--host") == 0 || strcmp(argv[arg_idx], "-h") == 0) {
            use_device_memory = false;
            arg_idx++;
        } else if (strcmp(argv[arg_idx], "--direct-doorbell") == 0) {
            use_direct_doorbell = true;
            arg_idx++;
        } else if (strncmp(argv[arg_idx], "--seed=", 7) == 0) {
            lfsr_seed = (uint32_t)strtoul(argv[arg_idx] + 7, NULL, 0);
            arg_idx++;
        } else {
            break;  /* Not a flag, must be positional arg */
        }
    }
    
    if (argc - arg_idx < 2) {
        fprintf(stderr, "Usage: %s [options] <nvme_device> <lba> [qid]\n", argv[0]);
        fprintf(stderr, "\n");
        fprintf(stderr, "Options:\n");
        fprintf(stderr, "  --host, -h            Use host memory (hipHostMalloc) [default]\n");
        fprintf(stderr, "  --device, -d          Use device memory (hipMalloc, VRAM P2P)\n");
        fprintf(stderr, "  --direct-doorbell     GPU writes directly to NVMe doorbell (passthrough only)\n");
        fprintf(stderr, "  --seed=HEXVAL         LFSR seed (default: 0xACE1)\n");
        fprintf(stderr, "\n");
        ...
        fprintf(stderr, "  4. GPU rings doorbell (via bridge or direct)\n");
        fprintf(stderr, "  5. Waits for completion\n");
        fprintf(stderr, "  6. Cleans up queue\n");
        fprintf(stderr, "\n");
        fprintf(stderr, "Examples:\n");
        fprintf(stderr, "  %s /dev/nvme0n1 1000                    # Via MMIO bridge\n", argv[0]);
        fprintf(stderr, "  %s --device /dev/nvme0n1 1000           # VRAM P2P via bridge\n", argv[0]);
        fprintf(stderr, "  %s --direct-doorbell /dev/nvme1n1 0     # Direct doorbell (passthrough)\n", argv[0]);
        fprintf(stderr, "  %s -d --direct-doorbell /dev/nvme1n1 0  # VRAM + direct doorbell\n", argv[0]);
        return 1;
    }
    
    nvme_dev = argv[arg_idx];
    lba = strtoull(argv[arg_idx + 1], NULL, 0);
    if (argc - arg_idx > 2) {
        qid = (uint16_t)strtoul(argv[arg_idx + 2], NULL, 0);
    }
    ...
    printf("  Queue ID:        %u\n", qid);
    printf("  Memory type:     %s\n", use_device_memory ? "Device (VRAM)" : "Host (pinned RAM)");
    printf("  Doorbell mode:   %s\n", use_direct_doorbell ? "Direct (GPU → NVMe BAR)" : "Via PCI MMIO Bridge");
    printf("  LFSR seed:       0x%08x\n", lfsr_seed);
    ...
    /* Step 3: Doorbell setup - Bridge or Direct */
    if (use_direct_doorbell) {
        /* Open DRM device for GPU registration */
        ...
        printf("Step 3: Mapping NVMe BAR0 for direct doorbell access...\n");
        nvme_bar0_cpu = map_nvme_bar0(nvme_bdf, drm_fd);
        if (!nvme_bar0_cpu) {
            fprintf(stderr, "ERROR: Failed to map NVMe BAR0!\n");
            fprintf(stderr, "  Try running without --direct-doorbell to use MMIO bridge\n");
            return 1;
        }
        printf("\n");
    } else {
    ...
    if (use_device_memory) {
        printf("Step 4: Allocating 4KiB data buffer in GPU VRAM (hipMalloc)...\n");
        /* Use hipMalloc for GPU VRAM allocation */
        if (hipMalloc(&data_buffer, NVME_PAGE_SIZE) != hipSuccess) {
            fprintf(stderr, "ERROR: hipMalloc failed\n");
            return 1;
        }
        
        printf("  VRAM buffer virtual: %p\n", data_buffer);
        printf("  Exporting as DMA-BUF (HSA runtime)...\n");
        
        /* Export VRAM as dmabuf */
        int dmabuf_fd = -1;
        uint64_t dmabuf_offset = 0;
        
        if (export_vram_as_dmabuf(data_buffer, NVME_PAGE_SIZE, &dmabuf_fd, &dmabuf_offset) < 0) {
            fprintf(stderr, "WARN: DMA-BUF export failed, falling back to host memory\n\n");
            hipFree(data_buffer);
            use_device_memory = false;
            goto use_host_memory;
        }
        
        /* Try to get physical address from dmabuf (via kernel P2PDMA) */
        if (get_dmabuf_physical_address(dmabuf_fd, nvme_bdf, dmabuf_offset, NVME_PAGE_SIZE, &data_phys) < 0) {
            fprintf(stderr, "\nWARN: Could not get physical address from VRAM dmabuf\n");
            fprintf(stderr, "      Falling back to host memory mode\n\n");
            close(dmabuf_fd);
            hipFree(data_buffer);
            use_device_memory = false;
            goto use_host_memory;
        ...
        close(dmabuf_fd);  /* Don't need fd anymore, kernel has the mapping */
        
        printf("  Data buffer physical: 0x%016lx (VRAM)\n", data_phys);
        printf("  GPU writes directly to VRAM\n");
        printf("  NVMe DMAs from VRAM (P2P)\n\n");
    }
...
use_host_memory:
    ...
    printf("Step 5: GPU fills buffer with LFSR pattern (seed=0x%08x)...\n", lfsr_seed);
    /* GPU writes to buffer (VRAM or pinned system RAM) */
    hipLaunchKernelGGL(gpu_fill_lfsr_pattern, grid, block, 0, 0,
                       (uint32_t*)data_buffer, NVME_PAGE_SIZE / sizeof(uint32_t), lfsr_seed);
    ...
    /* Show pattern */
    if (use_device_memory) {
        /* Need to copy from VRAM to see it */
        pattern = (uint32_t*)malloc(NVME_PAGE_SIZE);
        if (!pattern) {
            fprintf(stderr, "ERROR: malloc failed\n");
            return 1;
        }
        if (hipMemcpy(pattern, data_buffer, NVME_PAGE_SIZE, hipMemcpyDeviceToHost) != hipSuccess) {
        ...
        /* Direct access to pinned host memory */
    ...
    /* Verify GPU wrote correctly by reading back from VRAM */
    uint32_t verify_buf[8];
    if (use_device_memory) {
        hipMemcpy(verify_buf, data_buffer, 32, hipMemcpyDeviceToHost);
        printf("  GPU VRAM readback: 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x\n",
               verify_buf[0], verify_buf[1], verify_buf[2], verify_buf[3],
               verify_buf[4], verify_buf[5], verify_buf[6], verify_buf[7]);
        if (verify_buf[0] != lfsr_seed) {
            fprintf(stderr, "ERROR: GPU buffer verification failed!\n");
            fprintf(stderr, "       Expected first word: 0x%08x, got: 0x%08x\n",
                    lfsr_seed, verify_buf[0]);
            fprintf(stderr, "       GPU did not write pattern to expected VRAM location!\n");
            return 1;
        }
        printf("  ✅ GPU VRAM verification passed\n");
    }
    printf("\n");
    
    /* Step 6: Map MMIO bridge shadow buffer (if using bridge) */
    if (!use_direct_doorbell) {
        ...
        printf("Step 6: Skipping MMIO bridge (using direct doorbell)...\n\n");
    }
    
    /* Step 7-8: Register bridge or BAR0 with GPU */
    if (!use_direct_doorbell) {
        ...
        printf("Step 7-8: Registering NVMe BAR0 with GPU (already done in Step 3)...\n");
        /* Get GPU pointer to BAR0 */
        if (hipHostRegister(nvme_bar0_cpu, 8192, hipHostRegisterMapped) != hipSuccess) {
            fprintf(stderr, "ERROR: hipHostRegister failed for BAR0\n");
            return 1;
        }
        
        if (hipHostGetDevicePointer(&bridge_gpu, nvme_bar0_cpu, 0) != hipSuccess) {
            fprintf(stderr, "ERROR: hipHostGetDevicePointer failed for BAR0\n");
            return 1;
        }
        printf("  ✅ NVMe BAR0 accessible from GPU\n\n");
    }
    ...
    printf("  Mode: %s\n", use_direct_doorbell ? "Direct GPU → NVMe" : "GPU → Bridge → NVMe");
    ...
    hipLaunchKernelGGL(gpu_submit_nvme_write, dim3(1), dim3(1), 0, 0,
                       bridge_gpu,  /* Either bridge or NVMe BAR0 */
    ...
    if (use_direct_doorbell) {
        printf("  ✅ GPU rang doorbell directly to NVMe controller\n\n");
    } else {
    ...
    if (use_direct_doorbell) {
        printf("✅ GPU rang NVMe doorbell directly (P2P MMIO)\n");
    } else {
